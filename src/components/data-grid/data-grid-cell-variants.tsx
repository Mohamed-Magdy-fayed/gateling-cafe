...
export function SelectCell<TData>({
  ...
  const displayLabel =
    options.find((opt: CellSelectOption) => opt.value === value)?.label ?? value;
    ...
    >
      {isEditing ? (
        ...
        >
          ...
          >
            {options.map((option: CellSelectOption) => (
...
export function MultiSelectCell<TData>({
  ...
  const displayLabels = selectedValues
    .map((val) => options.find((opt: CellSelectOption) => opt.value === val)?.label ?? val)
    .filter(Boolean);
...
export function FileCell<TData>({
  ...
  const acceptedTypes = React.useMemo<string[] | null>(
