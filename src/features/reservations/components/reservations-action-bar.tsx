...
interface ReservationsActionBarProps {
  table: Table<Reservation>;
}

export function ReservationsActionBar({ table }: ReservationsActionBarProps) {
  ...
  const onReservationDelete = React.useCallback(async () => {
    setCurrentAction("delete");
    startTransition(async () => {
      const count = await deleteReservations(rows.map((row) => row.original.id));
    ...
    <DataTableActionBar table={table} visible={rows.length > 0}>
      ...
      <div className="flex items-center gap-1.5">
        ...
        <ModalSheetComponent
          ...
          confirmButton={
            <Button onClick={onReservationDelete}>{t("common.delete")}</Button>
          }
